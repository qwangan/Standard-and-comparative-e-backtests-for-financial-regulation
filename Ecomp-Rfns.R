...
sfEXP <- function(r,x,a,h=1)
{
  s <- vector(mode="numeric",length=length(r))
	if(h==1){
	  ind1 = (x>r)
	  ind2 = (x<r)
	  s[ind1] <- (-(1-2*a)*(x[ind1]-r[ind1])^2 + (1-a)*r[ind1]*(r[ind1]-2*x[ind1]))
	  s[ind2] <- ((1-a)*r[ind2]*(r[ind2]-2*x[ind2]))
	  }
	if(h==0){
	  ind1 = (x>r) & (r>0)
	  ind2 = (x<r) & (r>0)
...
#	eEVT=expectile(x, probs=tau) #empirical estimates
		...
		for (i in which(indEVT))
	{
		...
		if(Gfn.evt(u,tau=tau[i],dat=x,param)*Gfn.evt(ubnd,tau=tau[i],dat=x,param) <= 0){
...
plotTLM.ecomp <- function(Emat, rm, lev)
{
  x <- (Emat >= 10)*10 + (Emat<10 & Emat>=5)*5 + (Emat<5 & Emat>=2)*2 + (Emat < 2)
  y <- (Emat >= 2)*2 + (t(Emat) >= 2)
  m = dim(x)[1] # number of methods considered
  cellcol<-matrix(nrow=m)
  cellcol[is.na(x)]<-"white"
  #cellcol[x==3]<-"darkred"
  cellcol[x==10]<-"darkred"
  cellcol[x==5]<-"red"
  cellcol[x==2]<-"orange"
  cellcol[x==1]<-"green"
  cellcol[y==3]<-"yellow"
  ...
  par(mar=c(4,6,6,2)+0.1)
  color2D.matplot(x,cellcolors=cellcol, axes=FALSE, xlab="", ylab="")
  axis(side=3, at=(1:m)-0.5,labels=method.name, cex.axis=0.9, las=2)
  axis(side=2, at=(1:m)-0.5,labels=rev(method.name), cex.axis=0.9, las=2)
  mtext("internal model", side=3, line=4, cex=0.9)
  mtext("standard model", side=2, line=4, cex=0.9)
  mtext(xlb, side=1, line=1, cex=0.8)	
}
...
plotTLM.ecomp.thre <- function(Emat, rm, lev, thre)
{
  x <- (Emat >= thre)
  y <- (Emat >= thre) + (t(Emat) >= thre)
  z <- (Emat >= thre) + (t(Emat) >= thre) + (Emat > t(Emat))
  m = dim(x)[1] # number of methods considered
  cellcol<-matrix(nrow=m)
  cellcol[is.na(x)]<-"white"
  cellcol[x==1]<-"red"
